...
To import SARIF file findings,, simply execute the conviso findings import-sarif command in the terminal:

```conviso findings import-sarif --company-id 000 --input-file your_file.sarif```

This command allows integration of results from SARIF-compatible scanning tools into your Conviso Platform project, aiding in efficient vulnerability management.


### Options 
| Option                | Description                            |
| --------------------- | -------------------------------------- |
| --company-id TEXT   | env var: company-id (required)  |
...
| Secrets              | Description                                                                      |
| -------------------- | -------------------------------------------------------------------------------- |
| `CONVISO_API_KEY`       | Conviso Platform API Key (required)                                              |                                         |
    ...
    jobs:
      ...
      import:
        ...
        container:
          ...
          env:
            CONVISO_API_KEY: ${{secrets.FLOW_API_KEY}}
        steps:
          - name: Checkout code
            uses: actions/checkout@v3
